...
# Likelihood only on sampled observation days
for (t in (burn_in_timesteps + 1):(T_caseobs + burn_in_timesteps)) {
...
sample_indices <- sample(1:total_samples, size = 500, replace = FALSE)
...
total_delayed_summary <- as.data.frame(posterior_summaryb[grep("log10_conc", rownames(posterior_summaryb)), ])
...
# #Run the model with different initial values for each chain
# system.time({
#   Combined_WWd<- run.jags(textstring, data = dataListcomb,
#                           monitor = c("ww_pred","log10_conc_all","cases_pred",
#                                       "log10_conc","mu_nb","phi",
#                                       "P_total","A_total", "I_total","mult","log_mult",
#                                       "shed_P","shed_A","shed_I",
...
# })
# 
# Combined_WWd<- as.mcmc.list(Combined_WWd)
# save(Combined_WWd,file="U:/mpox25output/Combined_WWd.RData")

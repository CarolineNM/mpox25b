...
for (w in 1:T_WWobs) {
  cp_total[w] <- delayed_conc[ww_sample_days[w]]
...
system.time({
  Combined_casc<- run.jags(textstring, data = dataListcomb,
...
Combined_casc<- as.mcmc.list( Combined_casc)
save(Combined_casc,file="U:/mpox25output/Combined_casc.RData")

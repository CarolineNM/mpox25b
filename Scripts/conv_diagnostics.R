...
case_mod  <- Combined_case
ww_mod   <- Combined_WWe
com_mod <- Comblist_finalh
...
prior_definitions <- list(
  ...
  transit_time_mean = function(n) rtruncnorm(n, 1, 5, mean = 2.5, sd = sqrt(1)),
  transit_time_cv = function(n) rtruncnorm(n, 0.1, 1, mean = 0.3, sd = sqrt(1/3)),
  mult = function(n) exp(rnorm(n, log(3e-9), sqrt(1/40))),
...
plota=ggplot(beta_df_all %>% filter(parameter %in% c("transit_time_mean", "transit_time_cv", "mult","tau_ww")), 

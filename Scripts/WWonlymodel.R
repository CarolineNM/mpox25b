...
# Allow β > 1 but interpret β × contact as effective transmission rate per day,
# i.e., β × contact[g] becomes the true "per-day" risk, and β alone loses its strict probability meaning.
# In that case, β is not a true probability anymore, but a "rate modifier


##Parameterization of the Negative Binomial Distribution in JAGS:
##This model is similar to Negb25 fixed with the only difference being that I am estimating the initial conditions
##code similar to Negbin7 only difference being estimating initial conditions
##We are incorporating vaccination coverage as a rate not proportion
##we allow varying vaccination rates over discrete time steps
##Given proportions Pi,we solve inductively for each vi
...
##########Ww compartments#############
tau=0.5   ##relative infectiousness of Asymptomatic vs symptomatic(0.8 covid paper,mpox range 0.1-0.5)
...
inits_list <- list(
  list(beta = 0.8, kappa = 0.95, phi = 4, m = 0.06,
       transit_time_mean = 2.4, mult = 0.0005,
       .RNG.name = "base::Wichmann-Hill", .RNG.seed = 42),
  
  list(beta = 0.9, kappa = 0.92, phi = 5,m = 0.06,
       transit_time_mean = 2.6,mult = 0.0004,
...
system.time({
  WW_mod4<- run.jags(textstring, data = dataListWW,
...
WW_listmod4<- as.mcmc.list(WW_mod4)
save(WW_listmod4,file="U:/mpox25output/WW_listmod4.RData")
#########plot the output

load("Output/WW_listmod3.RData")
###generate traceplots
traceplot(WW_listmod3[, "transit_time_mean"],main="Mean transit time in sewer")
traceplot(WW_listmod3[, "transit_time_cv"],main="Standard deviation of transit mean time")
traceplot(WW_listmod3[, "mult"],main="Sacling factor of viral load")
traceplot(WW_listmod3[, "tau_ww"],main="Precision of the dnorm likelihood")
traceplot(WW_listmod3[, "beta"],main="Transmission parameter")
traceplot(WW_listmod3[, "kappa"],main="Mixing probability")
traceplot(WW_listmod3[, "phi"],main="Negative binomial dispersion parameter")
traceplot(WW_listmod3[, "report_frac"],main="reporting fraction")
traceplot(WW_listmod3[, "m"],main="Proportion of Asymptomatic fraction")


#####extract samples for plotting
chain_1_samples <- WW_listmod3[[2]]
mcmc_matrixall<-as.matrix(WW_listmod3)
...
posterior_summary[grep("beta|kappa|phi|transit_time_cv|mult|tau_ww|transit_time_mean", rownames(posterior_summary)), ]
...
plotprev=ggplot(plot_prev, aes(x = time)) +
  ...
  geom_line(aes(y = median_fit), color = "blue", size = 1) +                      # model median fit
  geom_ribbon(aes(ymin = lower_ci, ymax = upper_ci), fill = "lightblue", alpha = 0.5) +  # 95% CI
  labs(x = "Time", y = "Active prevalence", title = "Active prevalence-WW only model") +
  theme_minimal() +
  theme(legend.position = "top")

plotprev

CumInc_summaryb <- CumInc_summary[(burn_in_timesteps + 1):nrow(CumInc_summary), ]

plot_CumInc <- data.frame(
  time = 1:nrow(cases_obsb),                    # time index
  #Time=case_dat$Date,
  observed = cases_obsb,                    # observed cases
  median_fit = CumInc_summaryb$median,          # model median fit
  lower_ci = CumInc_summaryb$lower_95_CI,          # lower 95% CI
  upper_ci = CumInc_summaryb$upper_95_CI          # upper 95% CI
)

plot_CumInc=ggplot(plot_CumInc, aes(x = time)) +
  #geom_point(aes(y = observed), color = "black", size = 1) +  # observed cases
  geom_line(aes(y = median_fit), color = "blue", size = 1) +                      # model median fit
  geom_ribbon(aes(ymin = lower_ci, ymax = upper_ci), fill = "lightblue", alpha = 0.5) +  # 95% CI
  labs(x = "Time", y = "Cumulative incidence", title = "Cumulative incidence-WW only model") +
  theme_minimal() +
  theme(legend.position = "top")

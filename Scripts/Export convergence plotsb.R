...
load("D:/mpox25output/Comb_finaltestc.RData")
load("D:/mpox25output/Comb_WWtestc.RData")
load("D:/mpox25output/Combined_castestb.RData")

######generate the model fit
mcmc_matrixallWW<-as.matrix(Comb_WWtestc)
mcmc_matrixallcas<-as.matrix(Combined_castestb) ###most recent version
mcmc_matrixallcom<-as.matrix(Comb_finaltestc)
...
####################refine this######################
case_mod  <- Combined_castestb
ww_mod   <- Comb_WWtestc
com_mod <- Comb_finaltestc
...
prior_definitions <- list(
  ...
  transit_time_mean = function(n) rnorm(n, mean = 2.5, sd = sqrt(1 / 0.25)),
